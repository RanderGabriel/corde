...
Corde's configuration can be defined through a `corde.config.js|json|ts` file. This file must be at the root of your application, or you can declare a custom path for it with the cli option [--config](CLI.mdx#--config).
...
:::note both `cordeTestToken` and `botTestToken` can be found in [Discord Developers portal](https://discord.com/developers/applications) :::
